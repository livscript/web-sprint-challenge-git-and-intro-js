...
console.log("task 3:",artists[0].name)



//(2) Bio of the third artist (2nd index) in the array 

console.log("task 2:",artists[2].name)

/* 🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀 Task 2 (not auto tested): 🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀 
(no function needed) 
There is a typo in your dataset 😱 The 9th artist, Vincent Van Gogh is currently Vincent Van Dough. Fix this issue and console.log() to check your work. */

artists[8].bio = 'Vincent Van Doug'
...
function listOfNames(artistArr) {
  let artistArr2 = [...artistArr]
  for(let i = 0; i < artistArr.length; i++){
let currentArtistName = artistArr2[i].name
artistArr2[i] = currentArtistName
  }
  return artistArr2
}
...
artistArr2.push(newArtist);
return artistArr2
}
console.log("task 6:", addArtist(artists, "Oliwia Kwiatek", "2021-2022", "web dev", "Polish", " I'm an accountant, I love to invest in crypto and NFT's and I'd love to be a web dev" ))
...
function lotsOfArt(artistArr) {
  let lotsOfArtName = []
  for(let i = 0; i < artistArr.length; i++){
    let currentArtist = artistArr[i].paintings
    if(currentArtist > 100) {
      lotsOfArtName.push(artistArr[i].name)
    }

  }
  return lotsOfArtName
}
...
function artistInfo(artistArr, artistName){
for(let i = 0; i < artistArr.length; i++){
  let currentArtistName = artistArr[i].name
  if(currentArtistName == artistName){
    return artistArr[i].bio
  }
}
}
...
function artistByCountry(artistArr, nationality){
  let artistNames = []
  for(let i = 0; i < artistArr.length; i++){
    let currentNationality = artistArr[i].nationality
    currentNationality = currentNationality.split(',')
    if(currentNationality.length > 1) {
      continue
    }
  if(currentNationality[0] == nationality){
    artistNames.push(artistArr[i].name)
  }
  /*Your Code Here*/
}
return artistNames
